...
exports.up = function(knex) {
    return knex.schema
      ...
      .then(() => knex.schema.raw(`DROP TABLE IF EXISTS orders_detail CASCADE;`))
      .then(() => knex.schema.raw(`DROP TABLE IF EXISTS orders CASCADE;`))
      ...
      // Create orders table
      .then(() => knex.schema.createTable('orders', function(table) {
        table.increments('id').primary();
        ...
        table.string('shipping_method', 255);  // Shipping method
        table.string('tracking_number', 255);  // Shipping tracking number
        table.timestamp('complete_date');  // Order completion date
      }))
  
      // Create orders_detail table
      .then(() => knex.schema.createTable('orders_detail', function(table) {
        table.increments('id').primary();            
      ...
      .then(() => knex.schema.alterTable('orders', function(table) {
        table.foreign('customer_id').references('customer.customer_id');
        table.foreign('address_id').references('address.id');
      }))
      .then(() => knex.schema.alterTable('orders_detail', function(table) {
  ...
  exports.down = function(knex) {
    return knex.schema
      .dropTableIfExists('orders_detail')
